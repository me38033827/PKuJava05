...
public class dictionary1 {
	...
	public static void main(String[] args) {
		...
		try{                                        //读取错误抛出异常
			...
			in1.close();
			in2.close();
			//去掉标点符号  //在split后发现还有空格，用replaceAll来逐一去除特殊情况下的空格
			s1 = s1.replaceAll("\"","");
			s2 = s2.replaceAll("\"","");
			s1 = s1.replaceAll(", ",",");
			s2 = s2.replaceAll(", ",",");
			...
			int count=0;    //属于文件1且不属于文件2的单词数
			it=set2.iterator();
			while(it.hasNext()){
				String str=(String)it.next();         
				if(set3.contains(str)){              //将set3中的单词依次与set2比较，如果set2中存在，则跳过一次循环
					continue;
				}
				count++;
			}
			System.out.println("属于文件1且不属于文件2的单词占文件1的百分比为:"+((float)count/list1.size())*100+"%");
			count=0;            //属于文件2且不属于文件1的单词数
			it=set3.iterator();
			while(it.hasNext()){
				String str=(String)it.next();         
				if(set2.contains(str)){              //将set2中的单词依次与set2比较，如果set3中存在，则跳过一次循环
					continue;
				}
				count++;
			}
			System.out.println("属于文件2且不属于文件1的单词占文件2的百分比为:"+((float)count/list2.size())*100+"%");
